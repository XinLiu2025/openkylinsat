...
class MyFrame(wx.Frame):
    def __init__(self, parent, title):
        super(MyFrame, self).__init__(parent, title=title, size=(1400, 1000))
        ...
        # 设置背景图片
        bmp = wx.Bitmap("/home/nuc/桌面/NUC_openkylin/2.jpg")
        ...
        # 接口自动化检测
        self.port_automation_button = wx.Button(self.panel, label="接口自动化检测")
        self.port_automation_button.SetFont(font_big)
        # 上传云库
        self.port_upload_button = wx.Button(self.panel, label="上传云库")
        self.port_upload_button.SetFont(font_big)
        # 查看日志
        self.port_delete_button = wx.Button(self.panel, label="查看日志")
        self.port_delete_button.SetFont(font_big)

        # POC防逆向
        self.poc_automation_button = wx.Button(self.panel, label="POC防逆向")
        self.poc_automation_button.SetFont(font_big)
        # 访问POC
        self.poc_upload_button = wx.Button(self.panel, label="访问POC")
        self.poc_upload_button.SetFont(font_big)
        # 查看日志
        self.poc_delete_button = wx.Button(self.panel, label="查看日志")
        self.poc_delete_button.SetFont(font_big)
        ...
        self.output_text.SetBackgroundColour(wx.Colour(80, 80, 80, 100))
        ...
        qita_sizer = wx.GridSizer(3, 3, 10, 10)  # 4 行 3 列，水平和垂直间距为 10

        qita_sizer.Add(self.poc_automation_button, 0, wx.ALL | wx.EXPAND)
        qita_sizer.Add(self.poc_upload_button, 0, wx.ALL | wx.EXPAND)
        qita_sizer.Add(self.poc_delete_button, 0, wx.ALL | wx.EXPAND)

        qita_sizer.Add(self.port_automation_button, 0, wx.ALL | wx.EXPAND)
        qita_sizer.Add(self.port_upload_button, 0, wx.ALL | wx.EXPAND)
        qita_sizer.Add(self.port_delete_button, 0, wx.ALL | wx.EXPAND)
        ...
        self.poc_automation_button.Bind(wx.EVT_BUTTON, self.poc_automation)
        self.poc_upload_button.Bind(wx.EVT_BUTTON, self.poc_upload)
        self.poc_delete_button.Bind(wx.EVT_BUTTON, self.poc_delete)

        self.port_automation_button.Bind(wx.EVT_BUTTON, self.port_automation)
        self.port_upload_button.Bind(wx.EVT_BUTTON, self.port_upload)
        self.port_delete_button.Bind(wx.EVT_BUTTON, self.port_delete)
    ...
    def port_automation(self, event):
        self.run_file("/home/nuc/桌面/NUC_openkylin/接口自动化/port_auto.py")
    def port_upload(self, event):
        self.run_file("/home/nuc/桌面/NUC_openkylin/DB/portDB.py")
    def port_delete(self, event):
        webbrowser.open("http://152.136.142.183:39010/portAuto")

    def poc_automation(self, event):
        self.run_file("")
    def poc_upload(self, event):
        self.run_file("/home/nuc/桌面/NUC_openkylin/DB/portDB.py")
    def poc_delete(self, event):
        webbrowser.open("http://152.136.142.183:39010/portAuto")

    def run_file(self, file_path):
        current_dir = os.path.dirname(os.path.abspath(__file__))
        file_path = os.path.join(current_dir, file_path)
        process = subprocess.Popen(["/home/nuc/桌面/NUC_openkylin/venv/bin/python3.10", file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        while True:
            ...
            if output:
                ...
                self.output_text.AppendText(line)  # 每次只添加一行输出
                self.output_text.Refresh()  # 立即刷新文本框显示
