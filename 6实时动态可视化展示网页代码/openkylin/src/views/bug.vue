            ...
            <div class="left">
                <div style="font-size: 20px; font-weight: bold; display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <div style="padding: 10px; background-color: rgba(63, 150, 165, 0.1); margin-top: -10px;">
                      <span style="font-size: 24px; color: #FF5733;">风险评分：</span>
                  ...
                  <table class="baseline-result">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in bugResults" :key="index">
                        <td>{{ index + 1 }}</td>
                        <td>{{ item.name }}</td>  
                        <td><a :href="item.solution" target="_blank" style="font-size: 18px; color: inherit;">{{ item.solution }}</a></td>  <!-- 解决方案设置为超链接，并使字体大小和颜色与漏洞名称一致 -->
                      </tr>
                    </tbody>
                  </table>
...
export default {
  ...
  created() {
    ...
    .then(response => {
        ...
        this.score = calculateRiskScore(this.bugResults);  // 使用计算风险得分的函数来设置内核漏洞评分
      })
    .catch(error => {
    ...
    .then(response => {
        ...
        this.score1 = calculateRiskScore(this.bugResults1);  // 使用计算风险得分的函数来设置系统漏洞评分
      })
    .catch(error => {
